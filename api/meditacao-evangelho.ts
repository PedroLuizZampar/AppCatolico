...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // 1. Tentar buscar a meditação da data informada
    const result = await dbQuery(
      'SELECT conteudo, data FROM meditacoes_evangelho WHERE data = $1',
      [targetDate]
    );
    ...
    // 2. Fallback: Buscar a meditação mais recente se a de hoje não existir
    ...
    return res.status(404).json({
      error: 'Nenhuma meditação disponível no momento.'
    });
  } catch (error: any) {
    console.error('[API Meditacao] Erro ao obter dados:', error);
